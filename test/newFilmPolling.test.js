...
import { checkForNewFilms } from '../src/allocine/newFilmPolling.js';
...
const movieB = {
  ...
  sourceUrl: 'https://www.allocine.fr/film/fichefilm_gen_cfilm=2.html',
...
async function setup() {
  dir = await mkdtemp(join(tmpdir(), 'allocine-new-film-'));
...
test('a fetch failure keeps the previous baseline instead of throwing', async () => {
  ...
  await checkForNewFilms(
    gladys,
    async () => {
      throw new Error('allocine.fr unreachable');
    },
    state,
  );
...
test('a publishSceneEvent failure for one film does not stop the others from firing', async () => {
  ...
  const movieC = {
    ...
    sourceUrl: 'https://www.allocine.fr/film/fichefilm_gen_cfilm=3.html',
  };
  const state = buildState();
